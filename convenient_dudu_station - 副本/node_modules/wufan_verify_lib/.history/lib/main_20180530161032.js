...
var libs = {
    ...
    districts: districts,
    name: {//姓名校验
        validateNameChinese: function(str) {//校验中文字符数量
        ...
        validateNameEnglish: function(str) {//校验英文字符数量
            var result = { state: 1, content: "" };
        ...
        validateNameWithChineseAndEnGT2: function(value) {//校验一个以上、十一个以下中文字符
            var result = { state: 1, content: "" };
            // var patt1 = new RegExp(/^[\u4e00-\u9fa5]{2}[a-zA-Z]{4}$/); //验证只能中文输入
            if (
                validate.name.validateNameChinese(value).content <= 10 &&
                validate.name.validateNameChinese(value).content > 1
            ) {
            } else {
                result.content = "员工姓名必须含有一个以上、十一个以下中文字符，或者四位或以上英文字符，且不能含有阿拉伯数字或空格";
                result.state = -1;
            }
            return result;
        },
        validateNameWithENGT2: function(value) {//校验四位或以上英文字符
            var result = { state: 1, content: "" };
            var patt1 = new RegExp(/^[a-zA-Z\s]{4}$/); //验证只能中文输入
            if (validate.name.validateNameEnglish(value).content > 3) {
            } else {
                result.content = "员工姓名必须含有一个以上、十一个以下中文字符，或者四位或以上英文字符，且不能含有阿拉伯数字或空格";
                result.state = -1;
            }
                        ...
                        if (ic_number.length < 18) {
                            ...
                            if (this.checkIdCard(ic_number)) {
                                if (
                                    ...
                                    if (
                                        !this.checkSexAndId(ic_number, gender)
                                    ) {
                                        alertModal.open("证件号码与性别不匹配");
                                        return false;
                                    } else {
    ...
    checkIdNull: function(data) {
        if (
            data.ic_number ||
            (data.ic_number != null && data.ic_number != "")
        ) {
            return true;
        } else {
            return false;
    ...
    checkIcNumber: function(data) {
        ...
        if (data.ic_number == "" || !data.ic_number) {
            ...
            switch (data.ic_type) {
                case "H": //户口本
                    if (data.ic_number.length < 18) {
                        ...
                        if (this.checkIdCard(data.ic_number)) {
                            if (
                                data.ic_number.substring(6, 10) +
                                    "-" +
                                    data.ic_number.substring(10, 12) +
                                    "-" +
                                    data.ic_number.substring(12, 14) ==
                                data.birthday
                            ) {
                                if (
                                    !this.checkSexAndId(
                ...
                case "S": //士兵证
                    if (
                        data.ic_number.length >= 10 &&
                        data.ic_number.length <= 18
                    ) {
                        return true;
                    } else {
                        alertModal.open(
