...
var rules = {
    ...
    CheckVaildIcTime: function(ic_is_long_valid, ic_expiry_day) {
        var result = { state: 1, content: "" };
        var errmsg = "身份证有效期校验错误";
        if (ic_is_long_valid == 0 || ic_is_long_valid == "0") {
            if (ic_expiry_day) {
                if (tools.isDate(ic_expiry_day)) {
                    var now = new Date();
                    var dateNow =
                        now.getFullYear() +
                        "-" +
                        tools.MonthAndDay(now.getMonth() + 1) +
                        "-" +
                        tools.MonthAndDay(now.getDate());
    ...
    IcTypeAndCountryCheck: function(country, ic_type, age) {
        ...
        var errmsg = [
            "非中国籍不能选择居民身份证，请检查",
            "16周岁（含）以上中国大陆国籍被保人，证件类型必须为身份证、军官证、士兵证",
            "3周岁（含）-15周岁（含）中国大陆国籍被保人，证件类型必须为户口本、身份证",
            "0周岁（含）-2周岁（含）中国大陆国籍被保人，证件类型必须为出生证、户口本、身份证",
            "被保人国籍为香港、澳门，证件类型必须为港澳居民来往内地通行证",
            "被保人国籍为台湾，证件类型必须为台湾居民来往大陆通行证",
            "外籍被保人，证件类型必须为护照"
        ];
        var isValid = true;
        if (tools.checkCountryAndIcType(country, ic_type)) {
        } else {
            result.content = errmsg[0];
            result.state = -1;
            return result;
        }

        if (tools.checkCountryAndAgeLt16(country, ic_type, age)) {
        } else {
            result.content = errmsg[1];
            result.state = -1;
            return result;
        }

        if (tools.checkCountryAndAgeBetween3To16(country, ic_type, age)) {
        } else {
            result.content = errmsg[2];
            result.state = -1;
            return result;
        }

        if (tools.checkCountryAndAgeBetween0To3(country, ic_type, age)) {
        } else {
            result.content = errmsg[3];
            result.state = -1;
            return result;
        }

        if (tools.checkHongKongAndMacaoIcType(country, ic_type)) {
        } else {
            result.content = errmsg[4];
            result.state = -1;
            return result;
        }

        if (tools.checkTaiwanIcType(country, ic_type)) {
        } else {
            result.content = errmsg[5];
            result.state = -1;
            return result;
        }

        if (tools.checkOtherIcType(country, ic_type)) {
        } else {
            result.content = errmsg[6];
            result.state = -1;
            return result;
        }
    ...
    IcNumberAndTypeCheck: function(ic_type, ic_number, birthday, gender) {
        function CheckSexAndId() {
            var sex = rules.GetSexFromIdCard(ic_number).content;
            if (sex != gender) {
                return false;
            } else {
                return true;
        ...
        var result = { state: 1, content: "" };
        var errmsg = [
            "证件类型为身份证、户口本时,号码长度必须为18位",
            "证件号码与性别不匹配",
            "证件号码和出生日期不一致！请返回上一页确认正确的出生日期。",
            "请输入正确的证件号",
            "证件类型为护照时，号码长度不能少于3个字符",
            "证件类型为港澳居民来往内地通行证或台湾居民来往大陆通行证时，号码长度不能少于8个字符",
            "证件类型为军官证或士兵证，号码长度在10-18个字符之间",
            "证件类型为出生证,号码长度不能少于3个字符"
        ];
                    ...
                    if (ic_number.length < 18) {
                        ...
                        if (this.IdCardCheck(ic_number).state) {
                            if (
                                ...
                                if (!CheckSexAndId(ic_number, gender)) {
                                    result.content = [1];
                                    result.state = -1;
                                    return result;
                                } else {
